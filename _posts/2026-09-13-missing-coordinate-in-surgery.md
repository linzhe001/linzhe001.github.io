---
layout: post
title: "The Missing Coordinate in Surgery"
date: 2026-09-13 13:00:00 +0800
last_modified_at: 2026-09-13
meta: "Surgical Spatial Intelligence Series #1 · Version 1.0 · Published 2026-09-13"
description: "Why surgical spatial intelligence may begin with a map that knows when it is still valid"
tags: [surgical-spatial-intelligence, surgical-navigation, surgical-ai, medtech]
...
<h2>1. When the Coordinate System Changes</h2>
<p>Modern surgery is rich in images, instruments, and data. What it often lacks is a persistent spatial relationship between them. Consider an 8-millimeter ground-glass nodule: on a preoperative CT scan, its location may be clear—right upper lobe, 12 millimeters below the pleural surface. Once the lung is collapsed, rotated, and manipulated, the image remains available and the lesion is still there; what has changed is the coordinate system that connected the two.</p>
<p>Surgeons already have ways to recover that information. Percutaneous markers, microcoils, fluorescence, navigational bronchoscopy, intraoperative ultrasound, and cone-beam CT can each restore part of the spatial picture, but every method comes with its own workflow, hardware, timing, and clinical constraints. This points to a broader opportunity: some of the most valuable information in the operating room is not another image but the coordinate that makes an image actionable.</p>
<p>We use the term surgical spatial intelligence to describe the capability to restore a usable spatial relationship between preoperative anatomy and the intraoperative patient—and to maintain that relationship when needed. At full maturity, this capability could become a continuously updating map of deformable anatomy. The first useful product does not need to begin at that endpoint.</p>
<h2>2. Start with a Valid Moment</h2>
<p>The most ambitious form of soft-tissue navigation is continuous. The organ moves with respiration, tissue deforms under traction, and the field changes as structures are mobilized, divided, or resected. A mature system would track those changes, update the spatial model, recover after temporary loss of view, and estimate when its own state is becoming unreliable. That is the destination; the first useful product can solve a narrower problem.</p>
<p>Before an irreversible step—resection, transection, ablation, or deep dissection—the surgeon needs to know where the target is, where the critical surrounding structures are, and how much confidence to place in that spatial relationship. A system that can provide those answers at the moment of decision already creates clinical value. Once the anatomy has been substantially altered, that spatial state can expire; the system can then reacquire observations, register again, and establish a new state.</p>
<p>This creates a practical maturity path: point-in-time localization → stage-wise re-localization → continuous spatial state estimation. The first stage establishes a reliable state at the moment it matters. The second extends that capability across several important phases of the procedure. The third maintains the relationship continuously. Clinical value can therefore begin before continuous navigation is solved.</p>
...
<td>v1.0</td>
<td>2026-09-13</td>
<td>First published (bilingual, with an EN/ZH toggle; English default).</td>
</tr>
</tbody>
</table>
<blockquote>
<p><strong>Evidence update record</strong>: This is the anchor essay of the series. Sources and links will be added as Parts 02–05 are published; if any core judgment changes, it will be recorded here rather than silently revised.</p>
</blockquote>
</div>
...
<h2>1. 当坐标系发生变化</h2>
<p>现代手术室并不缺影像、设备和数据。真正缺少的往往是把这些信息连接起来的持续空间关系。以一颗 8 毫米的磨玻璃结节为例，术前 CT 可以清楚地显示它的位置：右上肺，距胸膜约 12 毫米。进入胸腔后，肺发生萎陷、旋转和牵拉。影像仍然存在，病灶也没有消失；改变的是两者之间原本那套坐标关系。</p>
<p>临床已经有很多方法重新获得位置信息。经皮标记、微线圈、荧光、导航支气管镜、术中超声和锥形束 CT，都能解决其中一部分问题，但每一种方法也都有自己的设备、流程、时间和适用范围。这指向了一个更基础的问题：手术室里最有价值的信息，有时不是另一张影像，而是让影像重新变得可用的那个坐标。</p>
<p>我把这种能力称为手术空间智能：在术中重新建立并在需要时维护术前解剖与当前患者之间可用的空间关系。它最成熟的形态可能是一张持续更新的软组织地图。但第一代真正可用的产品并不需要从这个终点开始。</p>
<h2>2. 从一个可信时刻开始</h2>
<p>连续导航是软组织空间智能最完整的形态。器官随着呼吸移动，组织在牵拉下发生形变，视野也会随着游离、离断和切除不断变化。成熟系统需要跟踪这些变化、更新空间模型、在短暂丢失视野后重新定位，并判断当前状态什么时候开始不可靠。这是长期目标，而第一代真正有用的产品可以先解决一个更窄的问题。</p>
<p>在切除、离断、消融或者深部解剖等不可逆操作之前，术者真正需要的是几个明确的答案：病灶现在在哪里，周围重要结构在哪里，以及当前这组空间关系有多可信。如果系统能够在这个决策点完成一次可靠的术前—术中配准，它已经产生了独立的临床价值。当组织被明显改变以后，当前空间状态可以结束，系统随后重新获取观测、重新配准，再建立一个新的状态。</p>
<p>由此可以形成一条清晰的能力路径：一次可信定位 → 关键阶段重新定位 → 连续空间状态。第一阶段解决的是在正确的时刻给出一次可信答案；第二阶段把这项能力延伸到若干重要的手术阶段；第三阶段才是持续维护位置、形变和不确定性的连续导航。临床价值不需要等到连续软组织导航完全解决以后才开始。</p>
<h2>3. 一张地图如何保持可信</h2>
...
<td>v1.0</td>
<td>2026-09-13</td>
<td>首次发布（中英对照，含中英切换，默认英文）。</td>
</tr>
</tbody>
</table>
<blockquote>
<p><strong>证据更新记录</strong>： 本文是系列总论，会随 02–05 篇发布补充来源与链接。若核心判断发生变化，会在此明确记录，而不是静默修改。</p>
</blockquote>
</div>
